...
async function applyLayout(server, world, layout, mcData, PBlock) {
    const touched = new Set();
    const note = (x, _y, z) => touched.add(`${x >> 4},${z >> 4}`);
    await Promise.all(dirtyPositions.map(p => {
        note(p.x, p.y, p.z);
        return world.setBlockStateId(new vec3_1.Vec3(p.x, p.y, p.z), 0);
    }));
    dirtyPositions = [];
        ...
        for (let x = region.min.x; x <= region.max.x; x++)
            for (let y = region.min.y; y <= region.max.y; y++)
                for (let z = region.min.z; z <= region.max.z; z++) {
                    ops.push(world.setBlockStateId(new vec3_1.Vec3(x, y, z), sid));
                    note(x, y, z);
                    if (region.block !== 'air')
                        dirtyPositions.push({ x, y, z });
                }
    ...
    await Promise.all(layout.placedBlocks.map(({ pos, block }) => {
        ...
        return world.setBlockStateId(new vec3_1.Vec3(pos.x, pos.y, pos.z), stateIdOf(mcData, PBlock, block));
    }));
    const chunks = Array.from(touched).map(k => {
        const [cx, cz] = k.split(',').map(Number);
        return { chunkX: cx, chunkZ: cz };
    });
    for (const oPlayer of server.players.filter((p) => p.world === world)) {
...
function buildPanelHtml(viewerPort) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
function startDevServer(options) {
    const { port, viewerPort = port + 1000, adminPort = port + 2000, version = '1.8.8', viewDistance = 4, testRunner, } = options;
    ...
    function runTest(suite) {
        ...
        broadcast(`\n$ npm run ${script}\n\n`);
        activeTest = (0, child_process_1.spawn)('npm', ['run', script], {
            cwd,
            env: { ...process.env, TEST_SERVER_PORT: String(port), ...extraEnv },
            shell: true,
        });
        activeTest.stdout.on('data', (d) => broadcast(d.toString()));
        activeTest.stderr.on('data', (d) => broadcast(d.toString()));
        activeTest.on('close', (code) => {
            broadcast(`\n[done] exit code ${code}\n`);
            broadcastEvent('done');
            activeTest = null;
        });
    ...
    server.once('listening', async () => {
        ...
        server.spawnPoint = new vec3_1.Vec3(0, 5, 0);
            ...
            if (req.method === 'GET' && url === '/') {
                ...
                req.on('end', async () => {
                    try {
                        ...
                        server.spawnPoint = new vec3_1.Vec3(layout.spawnPoint.x, layout.spawnPoint.y, layout.spawnPoint.z);
                        for (const player of Object.values(server.players)) {
                            if (player.username === 'Observer')
                                continue;
                            player.teleport(server.spawnPoint);
                        }
        ...
        const observerBot = mineflayer_1.default.createBot({
        ...
        observerBot.once('spawn', () => {
            ...
            const observer = Object.values(server.players).find((p) => p.username === 'Observer');
            if (observer)
                observer.teleport(new vec3_1.Vec3(8, 20, 8));
        });
    });
